...
スクリプトは `config/realsense/official/99-realsense-libusb.rules` を `/etc/udev/rules.d/`
...
`config/realsense/official/` 直下のファイルは上流のピン留め tag から **verbatim** で
vendored され、ドリフトチェックの基準になります。2 つの上流がこのフォルダに入ります
-- `realsense-ros` wrapper（`REALSENSE_ROS_VERSION` でピン留め）と `librealsense` SDK
（`LIBREALSENSE_VERSION` でピン留め）：
...
| `global_settings.yaml` | realsense-ros `realsense2_camera/CMakeLists.txt`（デフォルト `USE_LIFECYCLE_NODE=OFF` -> `use_lifecycle_node: false`） | `check_configs_sync.sh` |
| `d500_tables/*.json` | realsense-ros `realsense2_camera/examples/d500_tables/*.json` | （なし） |
