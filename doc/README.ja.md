...
`config/realsense/yaml/custom/none.yaml` という**空ファイル**で、runtime image は標準の
...
`config/realsense/` は **type-first** レイアウトで、**独自の** profile と
**vendored された上流**設定を分けて保管します：

```text
config/realsense/
├── yaml/
│   ├── official/   # vendored realsense-ros サンプル設定（ドリフトチェック）
│   └── custom/     # 独自のカメラ profile（none + 名前付き preset）
├── json/
│   └── official/   # vendored realsense-ros D500 サンプル JSON tables
...
#### `yaml/official/`、`json/official/`、`udev/` -- vendored 上流（手動編集しないこと）
...
| `yaml/official/global_settings.yaml` | realsense-ros `realsense2_camera/CMakeLists.txt`（デフォルト `USE_LIFECYCLE_NODE=OFF` -> `use_lifecycle_node: false`） | `check_configs_sync.sh` |
| `json/official/d500_tables/*.json` | realsense-ros `realsense2_camera/examples/d500_tables/*.json` | （なし） |
...
│   └── realsense/
│       ├── official/                  # 上流から verbatim vendored（ドリフト監視対象）
