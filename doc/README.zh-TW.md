...
腳本會把 `config/realsense/udev/99-realsense-libusb.rules` 複製到 `/etc/udev/rules.d/`
並重新載入 udev，之後請重新插拔相機。容器本身以 `privileged` 模式執行並掛載 `/dev`。

### 相機設定（Camera Config）

啟用中的相機 profile 由根目錄的 `camera.yaml` **symlink** 選定（比照
`app/ros1_bridge` 的 `bridge.yaml`）。預設目標是
`config/realsense/yaml/none.yaml`，一個**空檔**，因此 runtime image 會啟動
原廠上游預設（640x480x30，對齊深度），行為與先前完全一致。Dockerfile 會把
symlink 目標 COPY 成 `/camera_config.yaml`；當該檔非空時，entrypoint 會執行
`ros2 launch realsense2_camera rs_launch.py config_file:=/camera_config.yaml
initial_reset:=true`，否則執行預設 `CMD`。

啟用某個 profile 可以重指 symlink：
...
或不動 symlink，單次以 build arg 指定：

```bash
./script/build.sh --build-arg CAMERA_CONFIG=config/realsense/yaml/usb2_640x480p15fps.yaml
```

`config/realsense/` 採 **type-first** 佈局：

```text
config/realsense/
├── yaml/            # 我們的相機 profile（none + 具名 preset）
├── json/
│   └── d500_tables/ # vendored realsense-ros D500 範例 JSON tables
└── udev/            # vendored librealsense udev rules
```

vendored realsense-ros 漂移基準（`config.yaml`、`global_settings.yaml`）存放在
`.github/upstream-baseline/` -- 它是 CI 漂移 fixture，非使用者設定，也不會烘進 image。

#### `yaml/` -- 我們的 profile

每個解析度一個 preset，採該連線的最高 fps。**depth 一律 1280x720**（相機最高深度
解析度），上限 30 fps。每個 preset 皆關閉 infra（`enable_infra1/2`）與 IMU
（`enable_gyro/accel`），對齊深度維持開啟。`none.yaml` 是空的 0-byte 標記 =
原廠/預設（`camera.yaml` symlink 預設指向這裡）。
...
| `usb2_1280x720p6fps.yaml` | 1280x720x6 | 1280x720x6 | USB2（未驗證） |
| `usb2_640x480p15fps.yaml` | 640x480x15 | 1280x720x15 | USB2（未驗證） |
| `usb2_424x240p30fps.yaml` | 424x240x30 | 1280x720x30 | USB2（未驗證） |

USB3 preset 是在 D455 上以 `rs-enumerate-devices` 列舉得到。**USB2 preset 尚未
驗證**：當時相機接在 USB3 連線，未列舉 USB2 白名單 -- 720p depth 可能超出 USB2
頻寬或根本不提供。請在真實 USB2 連線上逐一驗證（D435/D455 接 USB 2 時無法承載
原廠 640x480x30 color + depth，30 fps 下送出 **0 frame**）。

#### `.github/upstream-baseline/`、`json/d500_tables/`、`udev/` -- vendored 上游（請勿手改）

vendored 檔案是自上游釘選 tag **verbatim** 複製，作為漂移檢查基準。有兩個上游 --
`realsense-ros` wrapper（由 `REALSENSE_ROS_VERSION` 釘選）與 `librealsense` SDK
（由 `LIBREALSENSE_VERSION` 釘選）：
...
| `.github/upstream-baseline/global_settings.yaml` | realsense-ros `realsense2_camera/CMakeLists.txt`（預設 `USE_LIFECYCLE_NODE=OFF` -> `use_lifecycle_node: false`） | `check_configs_sync.sh` |
| `json/d500_tables/*.json` | realsense-ros `realsense2_camera/examples/d500_tables/*.json` | （無） |
| `udev/99-realsense-libusb.rules` | librealsense `config/99-realsense-libusb.rules` | `check_udev_rules_sync.sh` |

`.github/workflows/upstream-bump.yaml` 會排程執行兩個漂移檢查，任一 vendored 副本偏離
其釘選上游時發出警告。上游升版時請重新 vendored，不要就地修改。
...
├── LICENSE
├── README.md
├── camera.yaml -> config/realsense/yaml/none.yaml # symlink（啟用中的相機設定；預設 = 原廠）
...
│   ├── check_configs_sync.sh    # 檢查 vendored 範例設定與釘選 realsense-ros tag 是否同步（repo 自有）
...
│       ├── yaml/                      # 我們自己的 profile（none + 具名 preset）
│       │   ├── none.yaml                 # 空檔 = 原廠/預設（camera.yaml 預設目標）
...
│       │   ├── usb2_1280x720p6fps.yaml   # USB2（未驗證）：color 1280x720x6，depth 1280x720x6
│       │   ├── usb2_640x480p15fps.yaml   # USB2（未驗證）：color 640x480x15，depth 1280x720x15
│       │   └── usb2_424x240p30fps.yaml   # USB2（未驗證）：color 424x240x30，depth 1280x720x30
│       ├── json/
│       │   └── d500_tables/          # vendored D500 範例 JSON 表（realsense-ros）
│       └── udev/
...
├── .github/
│   ├── upstream-baseline/       # vendored realsense-ros 漂移基準（config.yaml + global_settings.yaml；CI fixture，不烘進 image）
│   └── workflows/
