...
> **TL;DR** — 容器化的 Intel RealSense ROS 2 Humble 驅動程式。透過 apt 安裝 `realsense2_camera` 和 `librealsense2`，內含 udev 規則以存取裝置。
>
> ```bash
> ./build.sh && ./run.sh
> ```
...
- **Apt 安裝**：從 ROS 2 apt 套件庫安裝 `realsense2_camera` 和 `librealsense2`
...
# 2. 執行（預設：realsense2_camera rs_launch.py）
./run.sh
...
| `IMAGE_NAME` | 映像檔名稱 | `realsense_humble` |
...
| `runtime` | `ros:humble-ros-base-jammy` | RealSense 套件 + udev 規則 |
