...
腳本會把 `config/realsense/official/99-realsense-libusb.rules` 複製到 `/etc/udev/rules.d/`
...
`config/realsense/official/` 底下的檔案是自上游釘選 tag **verbatim** vendored，作為
漂移檢查基準。有兩個上游餵這個資料夾 -- `realsense-ros` wrapper（由
`REALSENSE_ROS_VERSION` 釘選）與 `librealsense` SDK（由 `LIBREALSENSE_VERSION` 釘選）：

| 檔案 | 上游來源 @ 釘選 tag | 漂移檢查 |
|------|----------------------|----------|
| `config.yaml` | realsense-ros `realsense2_camera/examples/launch_params_from_file/config/config.yaml` | `check_configs_sync.sh` |
| `global_settings.yaml` | realsense-ros `realsense2_camera/CMakeLists.txt`（預設 `USE_LIFECYCLE_NODE=OFF` -> `use_lifecycle_node: false`） | `check_configs_sync.sh` |
| `d500_tables/*.json` | realsense-ros `realsense2_camera/examples/d500_tables/*.json` | （無） |
