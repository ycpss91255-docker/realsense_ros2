...
`config/realsense/yaml/custom/none.yaml`，一個**空檔**，因此 runtime image 會啟動
...
`config/realsense/` 採 **type-first** 佈局，將**我們自己的** profile 與
**vendored 上游**設定分開存放：

```text
config/realsense/
├── yaml/
│   ├── official/   # vendored realsense-ros 範例設定（漂移檢查）
│   └── custom/     # 我們的相機 profile（none + 具名 preset）
├── json/
│   └── official/   # vendored realsense-ros D500 範例 JSON tables
└── udev/           # vendored librealsense udev rules
```

#### `yaml/custom/` -- 我們的 profile
...
#### `yaml/official/`、`json/official/`、`udev/` -- vendored 上游（請勿手改）
...
| `yaml/official/global_settings.yaml` | realsense-ros `realsense2_camera/CMakeLists.txt`（預設 `USE_LIFECYCLE_NODE=OFF` -> `use_lifecycle_node: false`） | `check_configs_sync.sh` |
| `json/official/d500_tables/*.json` | realsense-ros `realsense2_camera/examples/d500_tables/*.json` | （無） |
...
├── LICENSE
├── README.md
├── camera.yaml -> config/realsense/yaml/custom/none.yaml # symlink（啟用中的相機設定；預設 = 原廠）
...
│       ├── yaml/
│       │   ├── official/              # 自上游 verbatim vendored（受漂移檢查）
│       │   │   ├── config.yaml            # vendored 範例設定（realsense-ros）
│       │   │   └── global_settings.yaml   # vendored 範例設定（realsense-ros）
│       │   └── custom/                # 我們自己的 profile（none + 具名 preset）
