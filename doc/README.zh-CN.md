...
`config/realsense/yaml/none.yaml`，一个**空文件**，因此 runtime image 会启动
...
`config/realsense/` 采用 **type-first** 布局：

```text
config/realsense/
├── yaml/            # 我们的相机 profile（none + 具名 preset）
├── json/
│   └── d500_tables/ # vendored realsense-ros D500 示例 JSON tables
└── udev/            # vendored librealsense udev rules
```

vendored realsense-ros 漂移基准（`config.yaml`、`global_settings.yaml`）存放在
`.github/upstream-baseline/` -- 它是 CI 漂移 fixture，非用户配置，也不会烘进 image。
...
#### `.github/upstream-baseline/`、`json/d500_tables/`、`udev/` -- vendored 上游（请勿手改）
...
| `.github/upstream-baseline/global_settings.yaml` | realsense-ros `realsense2_camera/CMakeLists.txt`（默认 `USE_LIFECYCLE_NODE=OFF` -> `use_lifecycle_node: false`） | `check_configs_sync.sh` |
| `json/d500_tables/*.json` | realsense-ros `realsense2_camera/examples/d500_tables/*.json` | （无） |
...
├── LICENSE
├── README.md
├── camera.yaml -> config/realsense/yaml/none.yaml # 符号链接（启用中的相机配置；默认 = 原厂）
...
│       ├── yaml/                      # 我们自己的 profile（none + 具名 preset）
...
├── .github/
│   ├── upstream-baseline/       # vendored realsense-ros 漂移基准（config.yaml + global_settings.yaml；CI fixture，不烘进 image）
│   └── workflows/
