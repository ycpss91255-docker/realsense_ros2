...
`IntelRealSense/realsense-ros` at the tag pinned in the Dockerfile ARG
`REALSENSE_ROS_VERSION`, so they can serve as a drift-check baseline:
...
`.github/workflows/upstream-bump.yaml` runs a scheduled drift check
(`script/check_configs_sync.sh`) that diffs `config.yaml` / `global_settings.yaml`
against upstream at the pinned tag and warns when they diverge. Re-vendor on an
upstream bump instead of editing in place. (`99-realsense-libusb.rules` is
vendored the same way but keyed on `LIBREALSENSE_VERSION`, guarded by
...
│       ├── official/                  # vendored verbatim from realsense-ros (drift-checked)
